...
def read_config(configfile):
    config = configparser.ConfigParser({"external_efi":"False"})
...
def rollback_efi_partiton(efi_backup_dir,dry_run):
    full_path = efi_backup_dir+"/efi/"
    if dry_run:
        LOG.info("rsync -aq --delete {} /efi/".format(full_path))
    else:
        os.system("rsync -aq --delete {} /efi/".format(full_path))

def main():
    args = parse_args()
    config = read_config(args.config)
    
    if os.path.ismount("/efi") and config.get("root", "external_efi") == "False":
        LOG.warning("/efi is outside the `subvol_main`, your system may not boot after rollback!")
    ...
    try:
        ...
        exit(1)
    try:
        if config.get("root", "external_efi") == "True" and os.path.ismount("/efi"):
            efi_backup_dir = str(subvol_rollback_src) + config.get("root", "efi_backup_dir")
            rollback_efi_partiton(efi_backup_dir,dry_run=args.dry_run)
